...
4. Publish the nightly update with `powershell -NoProfile -ExecutionPolicy Bypass -File clips-worker/scripts/publish.ps1`. Never publish to the stable channel unless the user explicitly approves that specific stable promotion. Do not infer stable approval from a request to build, release, publish, or ship an update.
