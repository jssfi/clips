function displayVersion(value) {
  ...
  const developmentNightly = /^(\d+)\.(\d+)\.0-nightly\.(?:n0*(\d+)|(\d+))\.[0-9a-f]+$/i.exec(version);
  if (developmentNightly) return `${developmentNightly[1]}.${developmentNightly[2]}-nightly.${developmentNightly[3] || developmentNightly[4]}`;
